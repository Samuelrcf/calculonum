...
x1.append(x)
fxm.append(f(x))
w = int(input('Digite o valor do omega entre 0 e 1: '))
if w < 0 or w > 1:
    print('Você digitou um valor inválido.')
else:
    while abs(f(x)>precisao):
        x = x - (1 + w) * (f(x)/derivada(x))
        k1.append(k)
        x1.append(x)
        fxm.append(f(x))
        k=k+1
...
print(f"A raiz de f(x)=", y1, "é aproximadamente x=  ", x, "com o número de iterações igual a ", k2)